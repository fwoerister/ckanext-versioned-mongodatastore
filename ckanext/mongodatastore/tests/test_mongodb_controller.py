...
from ckanext.mongodatastore.controller.storage_controller import convert_to_csv, VersionedDataStoreController
...
class MongoDbControllerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.FIELDS = [
            {'id': 'distinct_field', 'type': 'number', 'info': None},
            {'id': 'field1', 'type': 'text', 'info': None},
            {'id': 'field2', 'type': 'number', 'info': None},
            {'id': 'id', 'type': 'number', 'info': None}
        ]
    ...
    def test_singleton(self):
        first_instance = VersionedDataStoreController.get_instance()
        second_instance = VersionedDataStoreController.get_instance()
    ...
    def test_reaload_config(self):
        ...
        first_instance = VersionedDataStoreController.get_instance()
        VersionedDataStoreController.reload_config(new_cfg)
        second_instance = VersionedDataStoreController.get_instance()
    ...
    def test_create_resource(self):
        mongo_cntr = VersionedDataStoreController.get_instance()
    ...
    def test_retrieve_stored_query(self):
        ...
        history_result = mongo_cntr.execute_stored_query(result[u'pid'], None, None)
        history_result_csv = mongo_cntr.execute_stored_query(result[u'pid'], None, None, 'csv')
    ...
    def test_retrieve_nonexisting_pid(self):
        mongo_cntr = VersionedDataStoreController.get_instance()
        self.assertRaises(QueryNotFoundException, mongo_cntr.execute_stored_query, 12344321, None, None)
